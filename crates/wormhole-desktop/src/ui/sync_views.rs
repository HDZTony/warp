...
impl View for SyncView {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        ...
        col.add_child(self.action_button("刷新", SyncAction::Refresh, false));
        col.add_child(ui_text::body(self.status.clone(), self.font).finish());
        col.add_child(ui_text::title("远程打开", self.font).finish());
        col.add_child(
            ui_text::body(self.remote_status.clone(), self.font)
                .with_color(theme::muted())
                .finish(),
        );
        ...
        col.add_child(ui_text::title("同步队列", self.font).finish());
