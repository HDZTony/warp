...
use wormhole_desktop_core::chat_rtc_call::{
    ...
    chat_voice_call_invite, chat_voice_call_status, ChatRtcCallStatusDto, ChatVideoCallDeviceParams,
    ChatVoiceCallConvParams, ChatVoiceCallInviteParams,
};
...
pub async fn fetch_status(core: &CoreHandle, conv_id: &str) -> Result<ChatRtcCallStatusDto, String> {
...
pub fn voice_status_to_header_line(
    phase: &str,
    remote_desktop_active: bool,
    peer_online: bool,
) -> String {
    if remote_desktop_active {
        return "远程桌面 · 已连接".into();
    }
    match phase {
        "ringing" => "正在呼叫…".into(),
        "incoming" => "来电".into(),
...
pub fn voice_error_toast(err: &str) -> (String, StatusTone) {
    ...
    if lower.contains("offline") || err.contains("离线") {
        ...
        ("无法访问麦克风，请检查权限与设备".into(), StatusTone::Danger)
