...
pub struct StubHarnessSupportClient;

#[cfg_attr(not(target_family = "wasm"), async_trait)]
#[cfg_attr(target_family = "wasm", async_trait(?Send))]
impl HarnessSupportClient for StubHarnessSupportClient {
    async fn create_external_conversation(&self, _format: &str) -> Result<AIConversationId> {
        anyhow::bail!("wormhole-slim: harness support unavailable")
    }
    ...
    fn http_client(&self) -> &http_client::Client {
        panic!("wormhole-slim: harness support HTTP client unavailable")
    }
}

pub type UploadBody = bytes::Bytes;

#[cfg(feature = "local_fs")]
pub use super::presigned_upload::FileUploadBody;

pub async fn upload_to_target<T>(
